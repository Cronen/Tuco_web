...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Reference
      .findOne(_.pick(params, _.keys(Reference.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const relations = _.pick(values, Reference.associations.map(ast => ast.alias));
    ...
    return Reference.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Reference.updateOne(params, data, { multi: true });

    // Update relational data and return the entry.
    return Reference.updateRelations(Object.assign(params, { values: relations }));
  },
  ...
  /**
   * Promise to search a/an reference.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Reference.attributes).reduce((acc, curr) => {
      switch (Reference.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
