...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Case
      .findOne(_.pick(params, _.keys(Case.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count cases.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Case.create(data);

    // Create relational data and return the entry.
    return Case.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Case.associations.map(a => a.alias));
    const data = _.omit(values, Case.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Case.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Case.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Case.attributes).reduce((acc, curr) => {
      switch (Case.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
