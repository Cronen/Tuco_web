...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return News
      .findOne(_.pick(params, _.keys(News.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await News.create(data);

    // Create relational data and return the entry.
    return News.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, News.associations.map(a => a.alias));
    const data = _.omit(values, News.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await News.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      News.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(News.attributes).reduce((acc, curr) => {
      switch (News.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
